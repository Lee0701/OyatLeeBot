...
const jeamfsList = function(str) {
  ...
  ALPHABET_LAYOUTS.forEach(alphabet => {
    HANGUL_LAYOUTS.forEach(hangul => {
      ...
      results.push({name: alphabet.name + ' - ' + hangul.name, str: result, count: count2350(result), score: count2350(result)*10 + countNumbers(result) + countRegex(result, REGEX_PARENTHESIS)*10})
    })
  })
  return results
}
